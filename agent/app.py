...
PRICING FALLBACK: When the AWS Pricing API does not return data for a service (e.g. Bedrock, AgentCore), use the fetch_aws_pricing_page tool to fetch the official pricing page directly. IMPORTANT: Use the Spanish version of the URLs as they contain static pricing data. For Bedrock models use "https://aws.amazon.com/es/bedrock/pricing/", for AgentCore use "https://aws.amazon.com/es/bedrock/agentcore/pricing/". Do NOT tell the user that pricing is unavailable without first trying to fetch the official page.
